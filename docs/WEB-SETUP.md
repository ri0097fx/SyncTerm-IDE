...
- **Watcher 側（PC B/C...）**  
  - `watcher_manager.sh` および `command_watcher.py`（または RT 版の `watcher_manager_rt.sh` / `command_watcher_rt.py`）を実行するマシン。  
...
- **Watcher 側**: SyncTerm-IDE の既存要件を満たし、`watcher_manager.sh` または `watcher_manager_rt.sh` を実行できること。
...
Watcher を稼働させる PC（PC B/C...）で、以下を実行する。
...
既に Watcher を運用している場合はそのまま利用できる。バックエンドは `sessions/` と `_registry/` を直接参照するため、Watcher 側の構成変更は不要である。

### 4-2. リバーストンネル版 Watcher（RT モード）

従来の `watcher_manager.sh` は rsync でコマンドを取得するため、数秒の遅延が生じる。**RT モード**では、Watcher が中継サーバーへ SSH リバーストンネルを張り、コマンドを HTTP で即時送受信する。
...
RT 版は `command_watcher_rt.py` および `watcher_manager_rt.sh` として別実装されている。既存の `command_watcher.py` / `watcher_manager.sh` は変更しない。
...
Watcher 側ではサーバー上のファイルが更新されただけとして扱われ、既存の `command_watcher.py` の動作に影響しない。
...
- `watcher_manager.sh`（または RT 版）が稼働しているか、`config.ini` の `base_path` が正しいかを確認する。
