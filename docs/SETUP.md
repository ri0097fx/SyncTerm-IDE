# SyncTerm-IDE セットアップ（デスクトップ版・廃止）

> **廃止**: デスクトップ版（Python/Tkinter GUI）は廃止されています。**通常は [WEB-SETUP.md](WEB-SETUP.md) に従って Web 版を利用してください。** デスクトップ版のコードは [../desktop_legacy/](../desktop_legacy/) に退避済みです。  
> 以下は過去のデスクトップ版向けセットアップ手順の参考用記載です。
