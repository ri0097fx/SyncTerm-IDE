...
- **README / docs**: Web 版を唯一の利用形態として記載。アーキテクチャ図を Browser + RT Watcher に更新。SETUP.md は廃止案内、USAGE/WEB-SETUP は RT のみに統一。
- **デプロイ**: `scripts/deploy_backend.sh` から `watcher_manager.sh` / `command_watcher.py` の転送を削除（RT 版のみデプロイ）。

### Fixed
- RT モードで relay にセッション dir が無い場合にキャッシュ削除が 404 で失敗する問題を修正（relay 側なしでも Watcher 側のみ削除可能に）。
...
### Added
- **Docker統合**: GUI から実行環境（Host / Docker Run / Docker Exec）を切り替え可能に。コンテナ自動作成やイメージ一覧取得もサポート。
- **ログ逐次同期**: ログ取得を別スレッド化し、実行中の出力をリアルタイムに近い形で表示可能に。
- **Python 実行ボタン**: エディタに「▶ Run」ボタンを追加。
- **シンボリックリンク対応強化**: リンク先フォルダ内でのファイル作成・削除に対応。

### Fixed
- Conda 表示修正: Anaconda がない環境でもプロンプトが正しく表示されるよう修正。

### Changed
- **大規模リファクタリング**: コードベースをモジュール化し、保守性を向上。これに伴い、実行コードが`gui_terminal.py`→`main.py`へと変更。
...
---
[Unreleased]: https://github.com/<your-account>/SyncTerm-IDE/compare/v2.3.1...HEAD
[2.3.1]: https://github.com/<your-account>/SyncTerm-IDE/compare/v2.3.0...v2.3.1
[2.3.0]: https://github.com/<your-account>/SyncTerm-IDE/compare/v2.2.2...v2.3.0
[2.2.2]: https://github.com/<your-account>/SyncTerm-IDE/compare/v2.2.1...v2.2.2
[2.2.1]: https://github.com/<your-account>/SyncTerm-IDE/compare/v2.2.0...v2.2.1
[2.2.0]: https://github.com/<your-account>/SyncTerm-IDE/compare/v2.1.1...v2.2.0
[2.1.1]: https://github.com/<your-account>/SyncTerm-IDE/compare/v2.1.0...v2.1.1
[2.1.0]: https://github.com/<your-account>/SyncTerm-IDE/compare/v2.0.0...v2.1.0
[2.0.0]: https://github.com/<your-account>/SyncTerm-IDE/releases/tag/v2.0.0
[1.0.0]: https://github.com/<your-account>/SyncTerm-IDE/releases/tag/v1.0.0
