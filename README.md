...
> 📣 What's New — v3.0.0: Docker 実行環境のサポート / 大規模リファクタリング / ログの逐次同期/ Python 実行ボタン の追加。

---

## Features

* **Docker 統合 (New)**:GUI から Docker Run (使い捨て) や Docker Exec (常駐) モードを切り替え可能。イメージやマウント設定も GUI で完結
* **ログの逐次同期 (New)**：別スレッドでのログ取得により、実行中のコマンド出力を逐次的に確認可能に
* **Python 実行ボタン (New)**：エディタ上のスクリプトをワンクリックで実行
* **マルチタブエディタ**：シンタックスハイライト、検索、スマートインデント、簡易補完
* **統合ターミナル**：Remote / Local 切替、履歴保持、Conda 環境の自動検知修正
...
* **リンク編集（Watcher 連携）**：リンク編集（Watcher 連携）: Watcher 上の任意フォルダを セッション直下にシンボリックリンクとして作成し、リンク先でのファイル作成・削除もサポート
...
* **Docker 実行モードについて**
  ...
  * `docker_run` モードはコマンドごとにコンテナを起動・破棄するため、環境変数は引き継がれません（クリーンな環境でのテスト用）。
  * `docker_exec` モードは常駐コンテナを使用するため、`pip install` 等の状態が保持されます。

* **リアルタイム性について**

  * ログの取得は 数ミリ秒〜数秒間隔のポーリング で行われます。完全なリアルタイムではありませんが、v3.0.0 よりバックグラウンドスレッドでの取得になったため、UI のフリーズは軽減されています。
