...
> 📣 **What's New — Extensions/Game 強化**: 拡張機能 Marketplace を拡張し、**Connect Four** を新規追加。あわせて **Sudoku Pro の視認性改善**、**Solitaire Klondike のクリック/キーボード操作強化**を実施。詳細は [CHANGELOG.md](CHANGELOG.md) を参照。
...
* **拡張機能 Marketplace / ランタイム**: ローカルカタログ + Sandbox Runtime により、DevTools / Productivity / Game 拡張を追加・有効化して使える。Quick Toggle、複数パネル表示、ドック移動（Preview↔Editor）にも対応。
* **ゲーム拡張の操作性強化**: ソリティア・数独・テトリス等を Web UI で直接操作。クリック/キーボードでプレイでき、ゲーム別の表示改善（配色・ハイライト・盤面UI）を反映。

---

## Extension Highlights（今回の更新）

### 新規追加
- **Connect Four (`game.board-connect-four`)**
  - 7x6 盤面、列クリックでドロップ
  - **AI 対戦 / 2P** 切替
  - キーボード `1-7` で列指定、`N` で新規、`M` でモード切替

### 改善
- **Sudoku Pro**
  - 固定数字/入力数字/カーソル/同一行列/同一3x3/エラーの配色を整理
  - 盤面コントラストを調整し、視認性を改善
- **Solitaire Klondike**
  - ボタン中心操作から、**クリック中心操作**へ改善
  - 選択→移動先クリック、ダブルクリック移動、キーボード操作を強化
